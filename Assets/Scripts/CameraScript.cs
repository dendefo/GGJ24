...
public class CameraScript : MonoBehaviour
{
    ...
    void Update()
    {
        var x = Mathf.Min(Mathf.Max(LevelManager.Instance.Zombie.transform.position.x + XOffset * cam.orthographicSize * cam.aspect, Boundries.xMin + (cam.orthographicSize * cam.aspect)), Boundries.xMax - (cam.orthographicSize * cam.aspect));
        var y = Mathf.Min(Mathf.Max(LevelManager.Instance.Zombie.transform.position.y + YOffset * cam.orthographicSize, Boundries.yMin + (cam.orthographicSize)), Boundries.yMax - (cam.orthographicSize));
